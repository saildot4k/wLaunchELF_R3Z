...
# Shared defaults.
# CI artifacts use the UDPFS network stack (not ETH/netfs+host).
ETH=0
UDPFS=1
EXFAT=1
MMCE=0
...
printf '%s\n' \
	"ETH=$ETH UDPFS=$UDPFS EXFAT=$EXFAT MMCE=$MMCE MX4SIO=$MX4SIO LCDVD=$LCDVD DVRP=$DVRP XFROM=$XFROM DS34=$DS34 DEBUG=$DEBUG_FLAG PPC_UART=$PPC_UART_FLAG"
