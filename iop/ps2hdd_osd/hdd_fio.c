...
static int getFileSlot(s32 device, apa_params_t *params, int checkDuplicate, hdd_file_slot_t **fileSlot);
...
static int getFileSlot(s32 device, apa_params_t *params, int checkDuplicate, hdd_file_slot_t **fileSlot)
{
    ...
    if (checkDuplicate) {
        for (i = 0; i < apaMaxOpen; i++) {
            if (hddFileSlots[i].f)
                if (hddFileSlots[i].device == (u32)device &&
                    memcmp(hddFileSlots[i].id, &params->id, APA_IDMAX) == 0)
...
static int apaRemove(s32 device, const char *id, const char *fpwd)
{
    ...
    for (i = 0; i < apaMaxOpen; i++) // look to see if open
    {
        if (hddFileSlots[i].f != 0) {
            if (hddFileSlots[i].device == (u32)device &&
                memcmp(hddFileSlots[i].id, id, APA_IDMAX) == 0)
                return -EBUSY;
        }
    }
...
static int apaRename(s32 device, const char *oldId, const char *newId)
{
    ...
    for (i = 0; i < apaMaxOpen; i++) {
        if (hddFileSlots[i].f != NULL) {
            if (hddFileSlots[i].device == (u32)device &&
                memcmp(hddFileSlots[i].id, oldId, APA_IDMAX) == 0) {
                SignalSema(fioSema);
                return -EBUSY;
            }
        }
...
int hddOpen(iomanX_iop_file_t *f, const char *name, int flags, int mode)
{
    ...
    WaitSema(fioSema);
    if ((rv = getFileSlot(f->unit, &params, !(f->mode & FIO_O_DIROPEN), &fileSlot)) == 0) {
        if (!(f->mode & FIO_O_DIROPEN)) {
            if ((rv = apaOpen(f->unit, fileSlot, &params, flags)) == 0) {
                fileSlot->device = f->unit;
                fileSlot->f = f;
                f->privdata = fileSlot;
            }
        } else {
