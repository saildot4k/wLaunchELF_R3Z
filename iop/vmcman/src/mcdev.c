...
#if defined(BUILDING_VMCMAN)
		if ((flags & (sceMcFileCreateFile | sceMcFileCreateDir)) != 0)
			printf("vmcman: mc_open McOpen name='%s' flags=0x%x ret=%d\n", filename, flags, r);
#endif
		if (r >= 0)
...
#if defined(BUILDING_VMCMAN)
	if ((flags & (sceMcFileCreateFile | sceMcFileCreateDir)) != 0)
		printf("vmcman: mc_open return raw=%d mapped=%d\n", r, mcman_ioerrcode(r));
#endif
